...
public class Sante extends javax.swing.JFrame {
    ...
    public boolean checkInput() {
        if (txtWeight.getText().trim().isEmpty() || txtTemp.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please fill in all required fields (Weight and Temperature).");
            return false;
        } else {
            return true;
        }
    ...
    private void btnSubmitActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnSubmitActionPerformed
        if (checkInput()) {
            String bloodP = txtBP.getText().trim().isEmpty() ? null : txtBP.getText();
            String bloodS = txtBS.getText().trim().isEmpty() ? null : txtBS.getText();
            if (bloodP != null) {
                // Vérification du format de Blood Pressure
                if (!bloodP.matches("\\d+/\\d+")) {
                    JOptionPane.showMessageDialog(this, "Blood pressure must be in the format 'systolic/diastolic' (e.g., 120/80).");
                    return;
                }
            }
            try {
                ...
                //check si l'utilisateyr déja inserer les infos
                String checkQuery = "SELECT COUNT(*) FROM infohealth WHERE id_user = ? AND date = ?";
                PreparedStatement checkStmt = conn.prepareStatement(checkQuery);
                checkStmt.setInt(1, userId);
                checkStmt.setDate(2, java.sql.Date.valueOf(today));
                ResultSet rs = checkStmt.executeQuery();
                if (rs.next() && rs.getInt(1) > 0) {
                    JOptionPane.showMessageDialog(this, "Data for today already exists.");
                    dispose();
                    HomeUser frame = new HomeUser();
                    frame.setVisible(true);
                    return;
                    ...
                    if (bloodS != null && (Float.parseFloat(bloodS) < 30 || Float.parseFloat(bloodS) > 600)) {
                        JOptionPane.showMessageDialog(this, "Blood Sugar value seems unrealistic. Please check.");
                        return;
                    }
                    // Vérifications supplémentaires
                    if (weight < 20 || weight > 300 || temperature < 30 || temperature > 55) {
                        JOptionPane.showMessageDialog(this, "Weight and temperature values seems unrealistic. Please check..");
                        return;
                    }
                    // Préparer la requête SQL
                    String insertQuery = "INSERT INTO infohealth (id_user, weight, temperature, blood_pressure, blood_sugar, date) "
                            + "VALUES (?, ?, ?, ?, ?, CURRENT_DATE())";
                    PreparedStatement insertStmt = conn.prepareStatement(insertQuery);
                    insertStmt.setInt(1, userId);
                    insertStmt.setFloat(2, weight);
                    insertStmt.setFloat(3, temperature);
                    if (bloodP != null) {
                        insertStmt.setString(4, bloodP);
                    } else {
                    ...
                    if (bloodS == null) {
                        ...
                        insertStmt.setFloat(5, Float.parseFloat(bloodS));
                    }
                    // Exécuter la requête
                    int rowsAffected = insertStmt.executeUpdate();
                    if (rowsAffected > 0) {
                        JOptionPane.showMessageDialog(this, "Health information saved successfully!");
                        dispose();
                        HomeUser frame = new HomeUser();
                        frame.setVisible(true);
                    } else {
                        JOptionPane.showMessageDialog(this, "Failed to save health information.");
                    }
                ...
                e.printStackTrace();
                JOptionPane.showMessageDialog(this, "An error occurred while saving health information.");
            }
            }
